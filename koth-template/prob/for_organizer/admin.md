...
429 Too Many Requests (3번째 연속 실패가 발생한 요청은 429 `TOO_MANY_ATTEMPTS`를 반환한다.)
...
- 락 상태에서 다시 요청하면 검증하지 않고 429를 반환합니다.
- `retry_after_seconds`는 현재 서버 시각 기준 남은 락 시간입니다.
- 검증 성공(`valid: true`) 시 연속 실패 횟수와 락 상태를 초기화합니다.
- 락 시간이 만료되면 다음 요청을 허용하며, 실패 횟수는 0으로 초기화합니다.
...
- 검증 제한(429)은 팀 토큰 추측을 통한 점수 조작을 막기 위한 것이며, 문제 서버는 429 응답을 받으면 참가자에게 대기 안내만 표시하고 재시도 로직으로 우회하지 않습니다.
