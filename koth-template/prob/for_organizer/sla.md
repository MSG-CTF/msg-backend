...
15분 점수 조회 시점에 서버가 unhealthy면 그 구간 점수가 반영되지 않습니다.

## 점수 반영

- 플랫폼 백엔드는 15분마다 문제 서버의 `GET /internal/koth/scores`를 호출합니다.
- 문제 서버는 `period_id` 기준으로 팀별 `metric_score`와 `period_rank`를 계산합니다.
...
- 백엔드는 응답의 `period_rank`에 플랫폼 배점표를 적용해 팀별 KOTH 누적 점수에 더합니다.
- `metric_score`는 순위 산정 원점수(Double)이고, `period_rank`는 해당 구간 등수(Long)입니다.
- 문제 서버는 점수를 반환하지 않습니다. 배점은 12문제 스케일을 맞추기 위해 플랫폼이 관리합니다.
- 전체 KOTH 점수는 플랫폼이 구간마다 지급한 점수의 합산으로 계산합니다.
