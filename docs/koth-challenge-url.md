...
- 같은 문제·15분 구간을 여러 작업이 동시에 수집해도 점수는 한 번만 반영한다.
  한 작업이 반영을 끝낸 뒤 다른 작업의 수집이 실패하더라도 완료 상태와 최초 결과를
  유지하며, 뒤늦은 요청은 이미 반영된 구간으로 처리한다.
- 점수 응답에는 `data` 키가 반드시 있어야 한다. 결과가 없는 구간의 `data: null`은
  정상으로 처리하지만, 키 자체가 누락되면 수집 실패로 기록한다.
- 잘못된 응답은 설정된 횟수만큼 재시도하고, 소진되면 다음 문제를 계속 수집한다.
  전체 처리가 끝난 뒤 실패한 문제가 있으면 명령은 실패 결과를 반환한다.
