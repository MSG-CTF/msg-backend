...
- 현재 호출 버전: `MSG-CTF/jm_devsecops@v3.3.1`
- CI 실행 시점: `main`으로 보내는 PR, `main`에 반영된 push
- CD(운영 배포): 아직 포함하지 않음
- 원격 브랜치: `chore/devsecops-v3`
- 검수 PR: `MSG-CTF/msg-backend#25`

중앙 DevSecOps 저장소의 `v3.3.1`은 일반 commit뿐 아니라 병합 commit의 각 부모
diff까지 Gitleaks 검사 범위에 포함합니다. 과거 `.env.save`에서 발견된 값은 더
이상 사용하지 않는 폐기 값으로 결정했고, 실제 값 대신 정확한 fingerprint만
`.gitleaksignore`에 기록했습니다. 경로나 탐지 규칙 전체는 무시하지 않습니다.
...
- 중앙 파이프라인은 `v3.3.1` 태그로 고정하여 중앙 `main` 변경이 백엔드 CI에
  갑자기 섞이지 않게 합니다.
...
Black은 저장소 전체에서 Python 파일 69개의 서식 차이를 보고하지만 현재 중앙
정책에서 병합 차단 항목은 아닙니다. 기능·보안 변경과 대규모 자동 서식 변경을 한
PR에 섞지 않기 위해 이번 브랜치에서는 자동 정렬하지 않았습니다.

## 6. 과거 시크릿과 `v3.3.1` 처리 결과

이전에 연결했던 `v3.3.0`의 기본 Gitleaks Git 검사는 일반 커밋 차이는 검사하지만,
...
1. 기존 값은 더 이상 사용하지 않는 폐기 값으로 결정했습니다.
2. Git history rewrite 대신 기존 기록을 유지합니다.
3. `v3.3.1`과 같은 merge-aware 검사로 137개 commit을 검사했습니다.
4. 12개 발견 중 중복을 제거한 정확한 fingerprint 10개만 `.gitleaksignore`에
   기록했습니다.
5. `.env.save` 경로나 `generic-api-key` 규칙 전체는 무시하지 않았습니다.
6. 중앙 `main`의 Actions를 통과한 commit에 `v3.3.1` 태그를 발행했습니다.
7. 백엔드 reusable CI 호출을 `@v3.3.1`로 변경했습니다.
8. 마지막으로 PR #25에서 merge-aware Gitleaks와 전체 CI를 다시 확인합니다.
...
- 과거 `.env.save` 값이 더 이상 어느 환경에서도 사용되지 않는지
- Git 기록을 유지하고 정확한 fingerprint만 예외 처리한 현재 방식을 승인할지
- 이 브랜치는 CI 준비만 포함하고 CD는 후속 작업으로 분리하는 것이 맞는지

## 8. 권장 진행 순서

1. PR #25에서 `v3.3.1` 전체 CI를 다시 실행합니다.
2. 과거 값이 모든 실제 환경에서 폐기됐다는 담당자 확인을 남깁니다.
3. PR에서 표시되는 실제 필수 검사 이름을 branch protection에 등록합니다.
   reusable 호출이므로 `ci / security-scan`, `ci / sast-scan`,
   `ci / lint-and-test`, `ci / docker-build-check` 형태로 표시됩니다.
4. 모든 필수 검사가 통과하고 리뷰 승인을 받은 뒤에만 `main`으로 병합합니다.
5. 병합 커밋과 `main` Actions 성공을 확인한 뒤 `chore/devsecops-v3` 브랜치를
   삭제합니다.
6. 새 PR이 기존 PR #1을 완전히 대체한 것을 확인하고 PR #1을 닫은 뒤
   `jm/devsecops-pipeline` 브랜치를 삭제합니다.

브랜치는 작업 내용을 운반하는 상자입니다. 내용이 `main`에 병합되고 검사까지
끝나기 전에 상자를 버리면 PR과 복구 경로가 사라집니다. 따라서 두 백엔드 작업
브랜치는 PR #25 병합이 끝난 뒤에만 삭제합니다.
