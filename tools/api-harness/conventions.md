...
- 주사위 보상은 보유량 3회까지만 지급한다. 이미 3회면 풀이 보상 지급 여부는 false이며 점수·마일리지·완료 기록은 정상 반영한다.
- 주사위 충전과 문제 개방 후 15분 보상 조건은 별개다. 보상·추가 소비 후에도 3회 미만이면 기존 `next_dice_reset_at`을 유지하고, 3회에 도달하면 null로 바꾼다. 이후 소비부터 새 15분을 계산한다.
...
- START(1번)는 소모하지 않으며, 2~36번의 35칸을 모두 소모하면 보드가 완료된다. 소모된 칸을 통과한 경로도 `movement_path`에 포함한다.
- START 통과 또는 도착 보상은 확정된 이동당 마일리지 100과 주사위 1회이며, 같은 요청 재시도에는 중복 지급하지 않는다. 보드 완료 후에는 자동 충전·추가 주사위 지급을 멈추고 `can_roll=false`, `blocked_reason=BOARD_COMPLETED`를 반환한다.
